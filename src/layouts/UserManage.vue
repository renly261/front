<template lang="pug">
  //- 管理者後台 layouts
  q-layout#userManage.shadow-2.rounded-borders(view='lHh Lpr lff' container style='height:100vh')
    ...
    q-drawer#drawer(v-model='drawer' show-if-above :width='200' :breakpoint='700' side='left')
      ...
      q-scroll-area(style='height: calc(100% - 150px); margin-top: 150px; border-right: 1px solid #ddd')
        q-list(padding)
          q-item(clickable v-ripple to='userMemberManage')
            q-item-section(avatar)
              q-icon(name='inbox')
            q-item-section 個人資料

          q-item(clickable v-ripple to='userOrderManage')
            q-item-section(avatar)
              q-icon(name='star')
            q-item-section 訂單管理
